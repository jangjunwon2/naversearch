...
> 최종 업데이트: 2026-06-05
> 상태: 핵심 기능 + **Discord 알림** 구현 완료, 로컬 동작 중 · 배포 방식 **라즈베리파이(3B+)** 로 확정(세팅 진행 예정)
...
│   ├── store.js           # JSON 저장소(history/forbidden/keyword-lists/schedules/notifications)
│   ├── scheduler.js       # 자동 주기 스캔 루프 + 스캔완료 알림·다이제스트 훅
│   ├── rankDiff.js        # 순위 비교 순수 함수(하락/이탈/목표미달, overallRank 기준)
│   ├── notifier.js        # Discord 웹훅 발송기(실패해도 throw 안 함)
│   ├── notifyEngine.js    # 알림 오케스트레이션(조건부 알림 + 매일 다이제스트)
│   ├── rateLimit.js       # 인메모리 IP 레이트리밋
│   ├── auth.js            # 비밀번호 게이트
│   └── loadEnv.js         # .env 로더(무의존)
├── lib/__tests__/         # rankDiff.test.js (node:test, 10케이스)
├── routes/                # companyScan·idScan·forbidden·keywords·keywordLists·schedules·history·auth·notifications
...
│                          # KeywordListControls·ScanProgress·AuthGate·NotificationSettings
```
...
- 알림: `GET/PUT /api/notifications`(설정·웹훅URL 미노출), `POST /api/notifications/test`(테스트 발송)
...
DISCORD_WEBHOOK_URL   # (선택) Discord 알림 웹훅 — 서버 전용 시크릿(API로 미노출)
TZ             # (선택) 권장 Asia/Seoul — 다이제스트 발송 시각 기준
...
npm test        # rankDiff 단위 테스트(node --test)
```
...
- **방식 확정(변경)**: **라즈베리파이 3B+** 자가호스팅 → 한국 IP(집 인터넷)로 네이버 차단 회피 · 24시간 상시 · 무료
  - 오라클 클라우드는 **가입/콘솔 에러로 사용 불가** → 보유 중인 라즈베리파이로 전환. (오라클 절차는 `ORACLE-DEPLOY.md`에 보존)
  - 가이드: **`RASPBERRY-PI-DEPLOY.md`** (OS 설치~PM2 상시 실행~Discord 알림 점검 전 과정)
- **하드웨어**: 메인 = Pi 3B+, 예비 = Pi 3B(동일 SD 이미지 1장 더 → 콜드 스페어)
- **1GB RAM 대응**: 프론트 빌드는 **PC에서 빌드 후 `frontend/dist`만 파이로 복사**(Vite OOM 회피), 파이엔 백엔드 의존성만 설치
- **접속/알림**: 자동 스캔·Discord 알림은 outbound라 포트포워딩 불필요. 외부 웹 UI 접속이 필요하면 **Cloudflare Tunnel**(무료)
- **준비물(기존)**: GitHub(`jangjunwon2/naversearch`), `.gitignore`, `DEPLOY.md`, 레이트리밋, 비밀번호 게이트
- **공개 정책**: 누구나 공개 + 내 API 키 공유 (→ 레이트리밋으로 보호, 외부 공개 시 `APP_PASSWORD` 권장)

---

## 4. 앞으로 할 일 (로드맵)

### 배포 마무리 (라즈베리파이 — `RASPBERRY-PI-DEPLOY.md` 따라 진행)
- [ ] Pi 3B+에 Raspberry Pi OS Lite(64-bit) 헤드리스 설치(Imager: SSH·WiFi·시간대 Asia/Seoul 사전 설정)
- [ ] Node 20 설치 → `git clone` + `.env`(API키·`DISCORD_WEBHOOK_URL`·`TZ=Asia/Seoul`)
- [ ] 프론트는 PC에서 `npm run build` → `frontend/dist`를 파이로 `scp` 복사, 파이엔 `npm install`(루트)
- [ ] `pm2 start backend/server.js` + `pm2 save` + `pm2 startup`(부팅 자동 시작)
- [ ] Discord 🔔 알림 탭 "테스트 발송"으로 폰 푸시까지 확인 + ⏰ 자동 스캔 대상 등록
- [ ] (선택) Cloudflare Tunnel로 외부 웹 UI 접속, (선택) Raspberry Pi Connect Lite로 원격 관리
- [ ] (선택) 예비 Pi 3B용 동일 SD 이미지 백업, `backend/store/*.json` 주기 백업
...
### 품질/운영
- [~] 테스트 코드: `rankDiff`(순위 비교) 단위 테스트 10케이스 완료 · 스크래퍼 파서·금칙어·포화지수는 미완
...
- 조건부 알림은 **자동 스캔에서만** 발송(수동 UI 스캔 제외), 하락/이탈 비교는 직전 동일 대상 스캔이 있어야 함
- 순위 비교 로직이 프론트(`MarketingDashboard.jsx`)와 백엔드(`rankDiff.js`)에 중복 존재 → 후속 통합 과제

---

## 6. 관련 문서
- `RASPBERRY-PI-DEPLOY.md` — 라즈베리파이 3B/3B+ 배포 전 과정(현재 배포 방식)
- `ORACLE-DEPLOY.md` — 오라클 클라우드 절차(보류, 참고용 보존)
- `DEPLOY.md` — 일반 배포 메모
- `docs/superpowers/specs/2026-06-05-discord-rank-notifications-design.md` — Discord 알림 설계
- `docs/superpowers/plans/2026-06-05-discord-rank-notifications.md` — Discord 알림 구현 계획
